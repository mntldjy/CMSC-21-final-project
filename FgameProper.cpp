...
FgameProper::FgameProper() {
}

// --- Main Game Logic ---
void FgameProper::run(Music& fightMusic) {
    srand(time(0));
    VideoMode desktop = VideoMode::getDesktopMode();
    int W = desktop.width;
    int H = desktop.height;
    RenderWindow app(desktop, "Fire Element", Style::Fullscreen);
    ...
    t2.loadFromFile("images/fireBG.jpg");
    t3.loadFromFile("images/explosions/type_C.png");
    t4.loadFromFile("images/meteors/yellow.png");
    t5.loadFromFile("images/bulletred.png");
                ...
                if (a->name == "asteroid" && b->name == "bullet")
                    if (isCollide(a, b)) {
                        ...
                        if (currentTime - lastHitTime <= 3.0f) {
                            hitStreak++;
                            score += 20; //score is increased by 10 pts per hit in a streak (10 + 10 * hitStreak)
                        }
                        else {
                            hitStreak = 1;
                            score += 10;
